...
internal sealed class ModConfig
{
    ...
    public string OpenKey { get; set; } = "K";
}
...
internal sealed class ModEntry : Mod
{
    ...
    private void OnButtonPressed(object? sender, ButtonPressedEventArgs e)
    {
        if (!Context.IsWorldReady)
            return;
        if (!Enum.TryParse(this.config.OpenKey, ignoreCase: true, out SButton openKey) || e.Button != openKey)
            return;
...
internal static class StockAggregator
{
    ...
    public static List<StockEntry> Collect()
    {
        ...
        var seenInventories = new HashSet<object>(ReferenceEqualityComparer.Instance);
