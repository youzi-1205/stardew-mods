...
「矿洞向导」「自动寻路」「背包扩充」「自动整理」支持 [Generic Mod Config Menu (GMCM)](https://www.nexusmods.com/stardewvalley/mods/5098)：装上它后，可在**标题画面齿轮按钮**或**游戏内设置菜单底部**用图形界面勾选开关、拖滑块调参数（采矿倍率、寻路/传送模式、背包格子数等），免去手动编辑 `config.json`。不装也不影响功能。
...
| `AutoSort` | **自动整理** | 一键把所有箱子按类别归整，同类集中、零散堆合并 |
...
| `MineHelper` | **矿洞向导** | 标出下楼梯子+敲指定石头出梯子；采矿掉落/采石掉煤概率/矿石刷新/洞穴凝胶钓获（可配） |
...
- **播种由玩家自己负责，帮工默认不补种**；**施肥只在箱子里有对应肥料时才进行**（没有就跳过、不空跑一趟）；肥料消耗自农场箱子，干完活回农舍旁角落待机

主要配置：`HelperCallKey`（`H`）、`HelperReplants`（默认 `false`，是否补种；播种默认交给玩家）、`HelperFertilizes`（默认 `true`，仅在箱中有对应肥料时才施肥）、`HelperSpeed`（8）、`HelperWorkSpeedMultiplier`（4.0）、`HelperAutoStartDaily`（true）、`HelperWorkRadius`（12）等。
...
一键把箱子里的物品按**大类**归整：同类集中到同一个箱子（矿石一箱、采集种植一箱、食物一箱……），零散的小堆顺便合并。

- **按 `L` 整理**：站在哪都行，立刻把作用范围内所有箱子重排一遍
- **箱子归属全自动认领**，无需任何手动设置：
  - 某箱子里现在装得最多的就是「矿石」，它就成为**矿石箱**；其它分类同理
  - 还没有归属箱的分类，先认领「已装最多该类」的箱子，再不行就认领一个空箱
  - 越用越稳定：归好一次后，每类都有固定的家
- **装满自动接力**：同一类装满一个箱子后，自动用旁边的空箱继续装——大仓库里一类可以横跨多个箱子
- **头顶标签**：每个分类箱上方自动显示它所装的分类名（写进存档、多人自动同步），一眼就知道哪箱装啥
- **分类（共 8 大类，对照游戏全部分类逐一归并）**：矿石宝石 / 采集种植（采集+蔬果+花+种子肥料） / 食物（鱼+蛋奶肉+熟食+食材） / 工匠品 / 资源材料（木石纤维+工艺材料+大型机器+怪物战利品） / 装备用品（武器+工具+戒指+靴子+帽子+衣服+饰品+鱼饵渔具） / 家具书籍 / 杂物（仅垃圾等真·杂项）。不会一个种类占一个箱子，也不会把鞋子衣服无脑丢进杂物
- **作用范围**：默认只整理农场、农场建筑内、农舍里的箱子（厨房冰箱不动）；可在配置里改为「全世界」
- **不会丢东西**：箱子彻底不够时，多出来的物品原地留在箱子里并提示「再加几个空箱子即可」；纯搬运箱子数据、瞬间完成，不需要角色走动；仅主机生效（多人安全）

配置：`SortKey`（`L`）、`SortEverywhere`（默认 false）、`SortOnChestClose`（关箱后自动整理，默认 false）、`SortOnDayStart`（每天早晨自动整理，默认 false）、`ShowSummary`、`ShowChestLabels`（头顶标签，默认 true）。装 GMCM 后可在游戏内菜单调整。
