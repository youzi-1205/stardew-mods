...
自用星露谷物语 SMAPI mod 合集，主打**减少重复劳动**：鹏神、自动钓鱼、跨图寻路、仓储一体化、矿洞辅助等。每个 mod 单一职责、互相独立，按需安装。
...
| 文件夹 | 显示名 | 一句话简介 |
|---|---|---|
| `FarmServant` | **鹏神** | 帮工 NPC：指挥他干全部农活 |
| `ChestHub` | **万能仓储** | 制作取料 + 箱子代付 + 一键补货 + 物品总览 |
| `AutoFish` | **自动钓鱼** | 全自动钓鱼与宝箱增强 |
| `AutoWalk` | **自动寻路** | 世界地图点击寻路/传送，跨地图自动奔跑 |
...
一个住在农场的帮工 NPC（默认名「鹏神」），真人式逐格走动干活。
...
4. **物品总览**：按 `K` 汇总背包+所有箱子+冰箱的每种物品总量，**按物品分类分组**、每项同时显示图标与名称；悬停查看存放分布，滚轮翻页
...
- **三选一模式**：自动寻路 / **传送**（点地图直接传送过去，不再走路）/ 关闭（仅查看地图）

配置：`OpenMapKey`（`M`）、`RunWhilePathing`、`StopOnMouseClick`、`Mode`（`Pathfind`/`Teleport`/`Off`）。装 GMCM 后可在游戏内菜单切换模式。
...
配置：`GuideToLadder`、`OreDropMultiplier`、`OreSpawnMultiplier` 及各矿种覆盖倍率。装 GMCM 后可在游戏内菜单拖滑块调整。
...
> ⚠️ 注意：此工具及仓库根目录的若干 `.ps1`/`.bat` 脚本是作者的个人工具，内部**硬编码了作者本机的游戏路径**（`D:\app\steam\...`），其他人直接运行会失败；需自行修改路径后使用。各 SMAPI mod 本体无任何硬编码路径，开箱即用。
...
- **多人联机**：鹏神核心逻辑运行在房主侧；制作取料在多人下未加箱子互斥锁，极端并发可能竞态
- 箱子代付的材料会短暂经过背包（占格子）；背包完全满时会提示清格
- 浣熊许愿（1.6 以物换物）暂未被箱子代付覆盖
- 矿洞向导的采矿掉落 / 矿石刷新倍率仅房主侧生效（多人客机靠存档同步）
